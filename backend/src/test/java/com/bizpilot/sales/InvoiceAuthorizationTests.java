...
/**
 * Mandatory Phase 11 RBAC tests: every {@code INVOICE_*} permission — newly
 * seeded by V9 — is enforced server-side on the matching operation. Mirrors
 * {@code QuotationAuthorizationTests}.
 *
 * <p>Seeded mapping recap (docs/security.md): EMPLOYEE = read-only;
 * SALES = CRUD minus delete; MANAGER/ADMIN/OWNER = full CRUD.
 */
...
class InvoiceAuthorizationTests {
    ...
    @Test
    void everyInvoiceEndpointRejectsUnauthenticatedRequests() {
        assertThat(restTemplate.getForEntity(url("/api/v1/invoices"), ApiError.class).getStatusCode())
                .isEqualTo(HttpStatus.UNAUTHORIZED);
        assertThat(restTemplate.postForEntity(url("/api/v1/invoices"),
                        new InvoiceCreateRequest(UUID.randomUUID(), null,
                                List.of(new InvoiceItemRequest(UUID.randomUUID(), BigDecimal.ONE))),
                        ApiError.class)
                .getStatusCode()).isEqualTo(HttpStatus.UNAUTHORIZED);
    }

    @Test
    void anEmployeeWithOnlyInvoiceReadCanReadButNotCreateUpdateOrDelete() {
        // EMPLOYEE is the default self-registration role — no promotion needed.
        String token = registerAndLogin("invoice-employee-rbac@example.com", "Employee RBAC Co");

        ResponseEntity<String> listResponse = getWithToken("/api/v1/invoices", token, String.class);
        assertThat(listResponse.getStatusCode()).isEqualTo(HttpStatus.OK);

        ResponseEntity<ApiError> createResponse = postWithToken("/api/v1/invoices",
                new InvoiceCreateRequest(UUID.randomUUID(), null,
                        List.of(new InvoiceItemRequest(UUID.randomUUID(), BigDecimal.ONE))), token, ApiError.class);
    ...
    void aSalesUserCanCreateAndUpdateButNotCancel() {
        String token = promotedToken("invoice-sales-rbac@example.com", "Sales RBAC Co", UserRole.SALES);
        UUID customerId = createCustomer(token, "Sales Customer").id();
        UUID productId = createProduct(token, "INVOICE-SALES-SKU").id();

        ResponseEntity<InvoiceResponse> createResponse = postWithToken("/api/v1/invoices",
                new InvoiceCreateRequest(customerId, null,
                        List.of(new InvoiceItemRequest(productId, BigDecimal.ONE))), token, InvoiceResponse.class);
        assertThat(createResponse.getStatusCode()).isEqualTo(HttpStatus.CREATED);
        String invoiceId = createResponse.getBody().id().toString();

        InvoiceUpdateRequest patch = new InvoiceUpdateRequest(
                null, null, true, null, null);
        HttpHeaders patchHeaders = new HttpHeaders();
        patchHeaders.setBearerAuth(token);
        ResponseEntity<InvoiceResponse> updateResponse = restTemplate.exchange(
                url("/api/v1/invoices/" + invoiceId), HttpMethod.PATCH,
                new HttpEntity<>(patch, patchHeaders), InvoiceResponse.class);
        assertThat(updateResponse.getStatusCode()).isEqualTo(HttpStatus.OK);

        HttpHeaders deleteHeaders = new HttpHeaders();
        deleteHeaders.setBearerAuth(token);
        ResponseEntity<ApiError> cancelResponse = restTemplate.exchange(
                url("/api/v1/invoices/" + invoiceId), HttpMethod.DELETE,
                new HttpEntity<>(deleteHeaders), ApiError.class);
        assertThat(cancelResponse.getStatusCode()).isEqualTo(HttpStatus.FORBIDDEN);
    }

    @Test
    void aManagerCanPerformEveryInvoiceOperationIncludingCancel() {
        String token = promotedToken("invoice-manager-rbac@example.com", "Manager RBAC Co", UserRole.MANAGER);
        UUID customerId = createCustomer(token, "Manager Customer").id();
        UUID productId = createProduct(token, "INVOICE-MANAGER-SKU").id();
    ...
    @Test
    void thePdfEndpointRequiresInvoiceReadPermission() {
        String managerToken = promotedToken("invoice-manager-pdf@example.com", "Manager Pdf Co", UserRole.MANAGER);
        UUID customerId = createCustomer(managerToken, "Pdf Customer").id();
        UUID productId = createProduct(managerToken, "INVOICE-PDF-SKU").id();
        ResponseEntity<InvoiceResponse> created = postWithToken("/api/v1/invoices",
                new InvoiceCreateRequest(customerId, null,
                        List.of(new InvoiceItemRequest(productId, BigDecimal.ONE))), managerToken,
                InvoiceResponse.class);
        String invoiceId = created.getBody().id().toString();
        ...
        String employeeToken = registerAndLogin("invoice-employee-pdf@example.com", "Employee Pdf Co");
        ResponseEntity<ApiError> response = getWithToken(
                "/api/v1/invoices/" + invoiceId + "/pdf", employeeToken, ApiError.class);
