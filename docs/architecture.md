...
- **Immutability once non-DRAFT** (`Quotation.isDraft()`, `QuotationNotEditableException`) — a production-readiness audit finding, added post-Phase-19. `QuotationService.update` originally had no status guard at all; this is now the same DRAFT-only-editable rule `InvoiceService.update` has had since Phase 11, applied consistently to its structurally identical sibling. See [security.md](security.md) §3r.
