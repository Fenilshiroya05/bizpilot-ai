...
> Status: Phase 12 — Tasks are implemented: the first resource with three independent cross-module reference validations on one resource (assignee, customer, lead — one more than Quotation/Invoice's two), and the first resource with a deliberately non-default RBAC role mapping (EMPLOYEE/SALES both get create/update, not just read). Reuses the tenant-safe reference-validation pattern from Phases 8/10/11 throughout. Adds new `TASK_*` permissions (V10, following the Phase 9/11 `PRODUCT_*`/`INVOICE_*` precedent). Email verification and forgot/reset-password are acknowledged CLAUDE.md requirements **not yet implemented** — deferred to a later authentication pass (see §1a).
