...
> Status: Phase 13 — Documents are implemented: the first resource involving real file I/O (local filesystem storage behind a swappable abstraction), the first with mandatory upload-content validation (extension + declared MIME + magic-byte signature), and the first hard-delete (no soft-delete/status trick) in the project. `DOCUMENT_READ`/`DOCUMENT_UPLOAD` already existed since Phase 6 (V4); only `DOCUMENT_DELETE` is new (V11), restricted to OWNER/ADMIN/MANAGER. No S3/MinIO implementation exists yet — local filesystem only, per CLAUDE.md §16's explicit Phase 13 scope; text extraction/chunking/embeddings/RAG are deferred to Phase 15. Email verification and forgot/reset-password are acknowledged CLAUDE.md requirements **not yet implemented** — deferred to a later authentication pass (see §1a).
